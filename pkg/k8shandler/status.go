...
func UpdateStatus(logging *v1alpha1.ClusterLogging) error {
	...
	if logging.Spec.LogStore.Type == "elasticsearch" {
		...
		if err != nil {
			return fmt.Errorf("Failed to get status for Elasticsearch")
		}
	...
	if logging.Spec.Curation.Type == "curator" {
		...
		if err != nil {
			return fmt.Errorf("Failed to get status for Curator")
		}
	...
	if logging.Spec.Collection.Type == "fluentd" {
		...
		if err != nil {
			return fmt.Errorf("Failed to get status of Fluentd")
		}
